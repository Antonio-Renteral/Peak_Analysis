...
1. **Desarrollo del Extractor de Secuencias**: Programación de la tarea que consiste en genera los archivos FASTA a partir del archivo de picos. Como es un proceso automatizado,
   todos la información requerida para ejecutar los programas debe ser por línea de comandos. Esto lo haremos mediante el uso de la libreria argparse que nos permitira manejar los
...
```bash
python main.py [--fasta_path RUTA_FASTA] [--peaks_path RUTA_PICOS] [--output_dir DIRECTORIO_SALIDA]
```

## Argumentos disponibles
--fasta_path: (Opcional) Ruta al archivo FASTA que contiene las secuencias genómicas.
Por defecto: ../data/E_coli_K12_MG1655_U00096.3.txt

--peaks_path: (Opcional) Ruta al archivo de picos en formato TSV.
Por defecto: ../data/union_peaks_file.tsv

--output_dir: (Opcional) Directorio donde se guardarán los resultados generados.
Por defecto: ../results
